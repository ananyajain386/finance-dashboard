...
export default function AddWidgetModal({ isOpen, onClose }) {
  ...
  const getFilteredAvailableFields = () => {
    ...
    if (displayMode === 'chart') {
      filtered = filtered.filter(isNumericField)
    }
  ...
  const handleSubmit = async () => {
    ...
    if (selectedFields.length === 0) {
      showError('Validation Error', 'Please select at least one field to display')
      return
    }
    ...
    await showSuccess('Widget Added!', `"${widgetName}" has been added to your dashboard.`, 2000)
    onClose()
  }

  const filteredFields = getFilteredAvailableFields()
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            {testResult && (
              ...
              >
                {testResult.message}
                {testResult.success && availableFields.length > 0 && (
                  <span className="ml-2">
                    {availableFields.length} top-level fields found.
                  </span>
                )}
              </div>
            )}
          ...
          <div>
            ...
            {displayMode === 'chart' && availableFields.length > 0 && (
              <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg text-sm">
                <svg className="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                Chart mode: Only numeric fields are shown for selection.
              </div>
            )}
          </div>
